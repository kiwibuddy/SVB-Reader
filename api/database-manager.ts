...
export class DatabaseManager {
  ...
  private async performInitialization(): Promise<void> {
    try {
      ...
      throw new Error(`Failed to initialize database after ${maxRetries} attempts. Last error: ${lastError}`);
    } catch (error) {
      console.error("Error initializing database:", error);
      
      // On Android, if all retries failed, try force reset
      if (Platform.OS === 'android' && this.initializationPromise) {
        ...
        try {
          ...
          throw new Error(`Database initialization failed even after reset. Original error: ${error}. Reset error: ${resetError}`);
        }
      }
