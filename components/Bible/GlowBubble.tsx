...
const GlowingBubble = ({
  ...
  // Arriving from a verse reference: wash the turn in the find tint, hold, then
  // let it go. Colour only — animating the border width would reflow the bubble
  // after the reader has already measured it to scroll here.
  useEffect(() => {
    if (!isTarget) return;
          ...
          <EmojiHandler block={block} blockIndex={bIndex} hasTail={hasTail} onLongPress={onLongPress}>
            <Animated.View
              style={[
                styles.bubble,
                radius,
                // bubbleStyle carries backgroundColor and borderColor so the
                // arrival pulse can animate them.
                bubbleStyle,
